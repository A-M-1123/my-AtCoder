...
def createRandomString(n)
  chars = ('a'..'z').to_a
  return n.times.map{chars.sample}.join
end

def getSordID(c)
  return c.ord - 'a'.ord
end
def getNextAlphbet(c)
  return nil if c == 'z'
  return (c.ord + 1).chr
end
def decSord(c)
...
N = getInt
s = gets.chomp.split("")
# s = createRandomString(N).chomp.split("") #test
@sord = Array.new(26,0)
for i in 0..(N-1)
  @sord[getSordID(s[i])] += 1
end
sordFirst = 'a'
while @sord[getSordID(sordFirst)] == 0
  sordFirst = getNextAlphbet(sordFirst)
end
...
while true
  ...
  while sordFirst != nil && @sord[getSordID(sordFirst)] == 0
    sordFirst = getNextAlphbet(sordFirst)
  end
  break if sordFirst == nil
  
  if s[st] == sordFirst
    decSord(s[st])
    st += 1
  elsif s[en] == sordFirst
    s[st],s[en] = s[en],s[st]
    decSord(s[st])
    decSord(s[en])
    st += 1
