...
sumR = Array.new(n+1,0)
sumC = Array.new(n+1,0)

posR = Array.new(n+1).map{Array.new(0)}
posC = Array.new(n+1).map{Array.new(0)}
...
rord = Array.new(n+1,0)
i = 0
while (i += 1) <= n
  rord[i] = i
end
rord.sort_by!{|e| m-sumR[e]}

ans = copyArr(ab)
ri = 0
while ri <= n
  ri += 1 if rord[ri] == 0
  if ri <= n && sumR[rord[ri]] < m
    r = rord[ri]
    c = r
    posi = 0
    (posR[r].empty?) ? (posL = 0):(posL = posR[r].length)
    while sumR[r] < m
      while sumC[c] == m || (posi < posL && posR[r][posi] == c)
        if posi < posL && posR[r][posi] == c
          posi += 1
        end
        c = (c+1) % (n+1)
        c += 1 if c == 0
