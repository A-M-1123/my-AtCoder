...
def div(n,mk1s,mk0s)
  rest = n-mk1s
  return rest if rest < mk1s+mk0s
  time = (rest-mk1s-mk0s)/mk1s
  rest -= mk1s*time
  return rest if rest < mk1s+mk0s
  return rest - mk1s
end

def solve(n,m,k,loop)
  return 0 if n == k && n+1 == m
  if n >= m
    mk0s = k
    mk1s = m-k
    return 0 if mk1s == 1
    ans2d = div(n,mk1s,mk0s)
    return loop[(ans2d-1)%4]
  else
    return loop[(n-1)%4]
  end
end
