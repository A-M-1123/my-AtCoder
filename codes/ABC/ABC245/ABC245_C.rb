...
n,k = gInts
a = gIntMat(2).transpose

dp = Array.new(n).map{Array.new(2,F)}
dp[0] = [T,T]
for i in 1..(n-1)
  for j in 0..1
    dp[i][j] = T if dp[i-1][0] && (a[i-1][0]-a[i][j]).abs <= k
    dp[i][j] = T if dp[i-1][1] && (a[i-1][1]-a[i][j]).abs <= k
  end
end
putsTF(dp[n-1][0]||dp[n-1][1], "Yes", "No")