...
maxT = t[-1]
dp = Array.new(maxT+1).map{Array.new(5,0)}

as = Array.new(maxT+1).map{Array.new(5,0)}
for i in 0..(n-1)
  as[t[i]][x[i]] += a[i]
end
...
for i in 1..maxT
  for j in 0..4
    ...
    for k in 0..4
      if j == k
        dp[i][j] = [dp[i][j], dp[i-1][k]+as[i][j]].max
      else
        delta = (j-k).abs
        dp[i][j] = [dp[i][j], dp[i-delta][k]+as[i][j]].max if 0 <= i-delta
      end
    end
  end
end
