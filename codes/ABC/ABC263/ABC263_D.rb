...
ax = Array.new(n).map{Array.new(2,0)}
ax[0] = [l,a[0]]
for i in 1..(n-1)
  ax[i][0] = ax[i-1][0] + l
  ax[i][1] = [ax[i-1][0]+a[i], ax[i-1][1]+a[i]].min
end

ay = Array.new(n).map{Array.new(2,0)}
ay[n-1] = [r,a[n-1]]
for i in 1..(n-1)
  id = n-i-1
  ay[id][0] = ay[id+1][0] + r
  ay[id][1] = [ay[id+1][0]+a[id], ay[id+1][1]+a[id]].min
end

putsArray2D(ax.transpose,2) if debug
putsArray2D(ay.transpose,2) if debug

min = 10**24
min = [min,ay[0].min].min
for i in 0..(n-2)
  min = [min,ax[i].min+ay[i+1].min].min
end
min = [min,ax[n-1].min].min
puts min