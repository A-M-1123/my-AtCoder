...
minNum = s.sort.join.to_i
maxNum = s.sort{|x,y| y <=> x}.join.to_i

s.sort!

ans = 0
(minNum > 0) ? (i = minNum**(0.5).to_i):(i = 0)
while true
  num = i**2
  len = getDigit(num)
  break if len > n || num > maxNum
  
  numArr = getNumApart(num)
  (n-len).times{numArr.push(0)} if n > len
  numArr.sort!
