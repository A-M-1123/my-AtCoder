...
def searchRow(st, en)
  ...
  len = en-st+1
  lft = len/2
  t = ask([st, st+lft-1, 1, @n])
  if t == lft
    return searchRow(st+lft, en)
  else
    return searchRow(st, st+lft-1)
  end
end
def searchCol(st, en)
  return st if st == en
  len = en-st+1
  lft = len/2
  t = ask([1, @n, st, st+lft-1])
  if t == lft
    return searchCol(st+lft, en)
  else
    return searchCol(st, st+lft-1)
  end
end
