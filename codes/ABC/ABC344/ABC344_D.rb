...
tLen = t.length
able = Array.new(tLen).map{Array.new(n).map{Array.new(a.max,false)}}
for st in 0..(tLen-1)
  for i in 0..(n-1)
    for j in 0..(a[i]-1)
      sLen = s[i][j].length
...
dp = Array.new(tLen,INF)
for i in 0..(n-1)
  newDP = Array.new(tLen,INF)
  for st in 0..(tLen-1)
    if 0 < st
      pre = dp[st-1]
      next if pre == INF
    ...
    for j in 0..(a[i]-1)
      if able[st][i][j]
        sLen = s[i][j].length
        newDP[st+sLen-1] = pre+1 if dp[st+sLen-1] > pre+1 && newDP[st+sLen-1] > pre+1
      end
    end
  end
  for j in 0..(tLen-1)
    dp[j] = newDP[j] if dp[j] > newDP[j]
  end
end
