...
sOrd = []
s.each{|c| sOrd << toNum(c)}
sNum = Hash.new(0)
sOrd.each{|so| sNum[so] += 1}
sNumArr = []
sNum.each do |key,val|
  sNumArr << [key,val]
end
sNumArr.sort!{|x,y| x[1] <=> y[1]}
sId = Hash.new(0)
sNumArr.each_with_index do |sn,id|
  key,val = sn
  sId[key] = id
end
for i in 0..(n-1)
...
sOrdRev = sOrd.reverse
maxIdStr = Array.new(n)
for i in 0..((n-1)/2)
  maxIdStr[i] = sOrdRev[2*i]
  maxIdStr[n-1-i] = sOrdRev[2*i+1] if 2*i+1 < n
end
maxId = maxIdStr.join.to_i
