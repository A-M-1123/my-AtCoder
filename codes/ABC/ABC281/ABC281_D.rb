...
dp = Array.new(n).map{Array.new(k).map{Array.new(d,-1)}}
dp[0][0][a[0]%d] = a[0]
for i in 0..(n-2)
  num = a[i+1]
  md = num % d
  dp[i+1][0][md] = [dp[i+1][0][md], num].max
  for j in 0..[k-1,i].min
    for m in 0..(d-1)
      if 0 <= dp[i][j][m]
        dp[i+1][j][m] = [dp[i+1][j][m], dp[i][j][m]].max
        newMd = (m+md) % d
        dp[i+1][j+1][newMd] = [dp[i+1][j+1][newMd], dp[i][j][m]+num].max if j < k-1
      end
    end
  end
end
