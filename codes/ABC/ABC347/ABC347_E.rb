...
class PFS
    ...
    def calc(l,r);l=check(l);r=check(r);return 0 if l>r;return @pfs[r+1]-@pfs[l];end
...
cnt = Hash.new(0)
sLen = Array.new(q,0)
ids = Array.new(n).map{Array.new(0)}
sl = 0
i = -1
while (i += 1) < q
  xi = x[i]
...
while (i += 1) < n
  ...
  j = 0
  while j < len/2
    a[i] += spfs.calc(ids[i][j*2],ids[i][j*2+1]-1)
    j += 1
  end
  a[i] += spfs.calc(ids[i][-1],q-1) if len % 2 == 1
end
puts a.join(" ")