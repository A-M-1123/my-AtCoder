...
aorg = a
borg = b
cd = c.to_s(2).split("").map(&:to_i)
len = cd.length
(60-len).times{cd.unshift(0)}
cd.reverse!
...
xpop = ad.sum
ypop = bd.sum
x = ad.reverse.join.to_i(2)
y = bd.reverse.join.to_i(2)
if xpop == aorg && ypop == borg && x^y == c
  puts "#{x} #{y}"
else
  puts -1
end