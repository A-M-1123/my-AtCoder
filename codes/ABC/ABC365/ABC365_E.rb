...
dLen = a.max.bit_length
dPfs = Array.new(n+1).map{Array.new(dLen,0)}
pfs = Array.new(n+1).map{Array.new(dLen).map{Array.new(2,0)}}
coef = Array.new(dLen,0)
coef[0] = 1
for i in 1..(dLen-1)
  coef[i] = coef[i-1]*2
end
...
while (i += 1) < n
  ...
  j = 0
  while 0 < tmp
    dPfs[i+1][j] = dPfs[i][j] + (tmp & 1)
    oe = dPfs[i+1][j] % 2
    pfs[i+1][j][oe] = pfs[i][j][oe] + 1
    pfs[i+1][j][(oe+1)%2] = pfs[i][j][(oe+1)%2]
...
while (i += 1) < n-1
  ...
  while (j += 1) < dLen
    oe = (dPfs[i][j] + 1) % 2
    ans += (pfs[n][j][oe]-pfs[i+1][j][oe])*coef[j]
    # puts "#{i},#{j} : oe=#{oe}, (#{pfs[n][j][oe]}-#{pfs[i+1][j][oe]})*2**i = #{(pfs[n][j][oe]-pfs[i+1][j][oe])*coef[j]}"
  end
end
puts ans