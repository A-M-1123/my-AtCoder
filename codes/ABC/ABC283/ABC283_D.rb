...
len = s.length
divN = 1
s.each{|e| divN += 1 if e == "("}
rngs = Array.new(divN).map{Array.new(0)}
rngNs = Array.new(divN,0)
sts = Array.new(divN,-1)
flag = T
cNs = Array.new(26,0)
id = 0
usedMaxID = 0
parentID = Array.new(divN,0)
for i in 0..(len-1)
  if s[i] != "(" && s[i] != ")"
    if sts[id] < 0
      sts[id] = i
  ...
  if s[i] == "("
    if 0 <= sts[id] && 0 <= i-1-sts[id]
      rngs[id].push([sts[id],i-1])
      rngNs[id] += 1
      sts[id] = -1
    end
    usedMaxID += 1
    parentID[usedMaxID] = id
    id = usedMaxID
  elsif s[i] == ")" || i == len-1
    if 0 <= sts[id] && 0 <= i-1-sts[id]
      rngs[id].push([sts[id],i-1])
