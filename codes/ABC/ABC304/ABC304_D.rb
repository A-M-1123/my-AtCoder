...
def bins(a, st, en, tar)
  mid = (st+en) / 2
  piv = a[mid]
  if st == en || (piv <= tar && tar < a[mid+1])
    return mid
  elsif tar < piv
    return bins(a, st, mid-1, tar)
  else
    return bins(a, mid+1, en, tar)
  end
end
...
pos = Array.new(N).map{Array.new(2)}
a.unshift(0)
a.push(W)
b.unshift(0)
b.push(H)
...
for i in 0..(N-1)
  p,q = pq[i]
  pos[i][0] = bins(aCmp,0,aCLen-1,p)
  pos[i][1] = bins(bCmp,0,bCLen-1,q)
end

poss = Array.new((aCLen-1)*(bCLen-1),0)
for i in 0..(N-1)
  id = pos[i][0] + pos[i][1]*(aCLen-1)
  poss[id] += 1
end
poss.sort!
ansMin = poss[0]
