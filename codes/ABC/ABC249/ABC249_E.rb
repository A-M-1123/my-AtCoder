...
dp = Array.new(n).map{Array.new(n).map{Array.new(3,0)}}
dp[0][0][0] = 1
for i in 0..(n-2)
  for j in 0..i
    dp[i+1][j][1] = (dp[i+1][j][1] + dp[i][j][0]) % p
    dp[i+1][j+1][0] = (dp[i+1][j+1][0] + dp[i][j][0]) % p
    dp[i+1][j][2] = (dp[i+1][j][2] + dp[i][j][1]) % p
    dp[i+1][j+1][0] = (dp[i+1][j+1][0] + dp[i][j][1]) % p
    dp[i+1][j][2] = (dp[i+1][j][2] + dp[i][j][2]) % p
    dp[i+1][j+1][2] = (dp[i+1][j+1][2] + dp[i][j][2]) % p
  end
end
