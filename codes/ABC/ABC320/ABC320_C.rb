...
m = gInt
sTmp = 3.times.map{gets.chomp.split("").map(&:to_i)}

s = Array.new(3).map{Array.new(0)}
for i in 0..2
  for time in 0..2
    for j in 0..(m-1)
      s[i].push(sTmp[i][j])
...
ss = Array.new(3).map{Array.new(10).map{Array.new(0)}}
ssLen = Array.new(3).map{Array.new(10,0)}
...
ans = INF
for num in 0..9
  if ssLen[0][num] >= 3 && ssLen[1][num] >= 3 && ssLen[2][num] >= 3
    tmp = []
    for i in 0..2
      for j in 0..2
        tmp.push([ss[i][num][j], i])
      end
    end
    tmp.sort{|x,y| (x[0] <=> y[0]).nonzero? || x[1] <=> y[1]}
