...
def makeArrVal(*arg);len=arg.length;arr=Array.new(arg[-1],arg[0]);i=len-2;while 0<i;arr=Array.new(arg[i]).map{arr};i-=1;end;return arr;end
# makeArray(val,dim*)
...
def sort2DArray(arr)
  arr.sort!{|x,y| (x[0] <=> y[0]).nonzero? || x[1] <=> y[1]}
end
...
h,w = w,h if h > w
for i in 0..(n-1)
  ab[i][0],ab[i][1] = ab[i][1],ab[i][0] if ab[i][0] > ab[i][1]
end
...
[0, 1].repeated_permutation(n) do |bit|
  ...
  useIDs = []
  for i in 0..(n-1)
    useIDs.push(i) if bit[i] == 1
  end
  sum = 0
  for i in 0..(useIDs.length-1)
    id = useIDs[i]
    sum += ab[id][0]*ab[id][1]
  end
  if sum == h*w
...
for i in 0..(idsN-1)
  len = ids[i].length
  ids[i].permutation do |idp|
    [0, 1].repeated_permutation(len) do |bit|
      isPass = F
      for j in 0..(len-1)
        if isSq[idp[j]] && bit[j] == 1
          isPass = T
          break
        end
      end
      next if isPass
      tl = Array.new(h).map{Array.new(w,F)}
      id = 0
      flag = T
      for y in 0..(h-1)
        for x in 0..(w-1)
          if !tl[y][x]
            tid = idp[id]
            (bit[id] == 0) ? (tlh,tlw = ab[tid]):(tlw,tlh = ab[tid])
            for ty in y..(y+tlh-1)
              for tx in x..(x+tlw-1)
                if h <= ty || w <= tx || tl[ty][tx]
                  flag = F
                  break
                else
                  tl[ty][tx] = T
