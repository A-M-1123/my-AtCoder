...
gcnt = 0
gs = []
aid = Array.new(h).map{Array.new(w)}
for y in 0..(h-1)
  for x in 0..(w-1)
    aid[y][x] = y*w + x
    if s[y][x] == "#"
...
for y in 0..(h-1)
  for x in 0..(w-1)
    ...
    gid = []
    if 0 <= y-1 && s[y-1][x] == "#"
      gid.push(iding[aid[y-1][x]])
      around += 1
    end
    if 0 <= x-1 && s[y][x-1] == "#"
      gid.push(iding[aid[y][x-1]])
      around += 1
    end
    if y+1 < h && s[y+1][x] == "#"
      gid.push(iding[aid[y+1][x]])
    ...
    if around == 0
      ...
      pars = []
      gid.each{|e| pars.push(uf.get_parent(e))}
      vali = pars.sort.uniq.length
      sum += blc-vali+1
    end
  end
end
gcd = sum.gcd(try)
ans = fracMod(sum/gcd, try/gcd, MOD)
puts ans