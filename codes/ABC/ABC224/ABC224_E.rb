...
h,w,n = getInts
r,c,a = getIntMatVars(n)

hcnt = Array.new(h,0)
wcnt = Array.new(w,0)

rca = Array.new(n).map{Array.new(4)}
for i in 0..(n-1)
  rca[i] = [r[i],c[i],a[i],i]
end
...
for i in 0..(n-1)
  ...
  if ai != prevn
    stock.each do |e|
      hcnt[e[0]] = [hcnt[e[0]], e[2]].max
      wcnt[e[1]] = [wcnt[e[1]], e[2]].max
    end
    stock = []
    prevn = ai
  end
  
  cnti = [hcnt[ri-1], wcnt[ci-1]].max
  ans[ii] = cnti
  cnti += 1
  stock.push([ri-1,ci-1,cnti])
  # hcnt[ri-1] = cnti
  # wcnt[ci-1] = cnti
end
