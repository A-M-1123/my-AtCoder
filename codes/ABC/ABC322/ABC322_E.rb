...
def unite(arr,valMax)
  ret = 0
  for i in 0..(arr.length-1)
    ret += arr[i]*((valMax+1)**i)
  end
  return ret
end
def apart(num,len,valMax)
  return num.to_s(valMax+1).rjust(len, '0').reverse.split("").map(&:to_i)
end
...
maxVal = (p+1)**k - 1
dp = Array.new(n).map{Array.new(maxVal+1,INF)}
dp[0][unite(a[0],p)] = c[0]
for i in 0..(n-2)
  for j in 0..maxVal
    if j == 0
      dp[i+1][unite(a[i+1],p)] = [dp[i+1][unite(a[i+1],p)], c[i+1]].min
    elsif dp[i][j] < INF
      addedStat = apart(j,k,p)
      for x in 0..(k-1)
        addedStat[x] = [addedStat[x]+a[i+1][x], p].min
      end
      uStat = unite(addedStat,p)
      dp[i+1][j] = [dp[i+1][j], dp[i][j]].min
      dp[i+1][uStat] = [dp[i+1][uStat], dp[i][j]+c[i+1]].min
    end
  end
end
ans =  dp[n-1][maxVal]
...
=begin
for i in 0..maxVal
  print "#{apart(i,k,p).join("")} :"
  for j in 0..(n-1)
    (dp[j][i] < INF) ? (print " #{dp[j][i]}"):(print " --")
  end
  puts ""
end
=end