...
for i in 0..2
  p[i] = trim2DArray(p[i],trimPos2DArray(p[i],"#"))
end
pWH = Array.new(3).map{Array.new(2)}
for i in 0..2
  pWH[i][0] = p[i][0].length
  pWH[i][1] = p[i].length
end

pTurns = Array.new(3).map{Array.new(4)}
for i in 0..2
  for j in 0..3
    pTurns[i][j] = turn2DArray(p[i],"r",90*j)
  end
end

pVal = Array.new(3).map{Array.new(0)}
for id in 0..2
  for type in 0..3
    (type % 2 == 0) ? (w,h = pWH[id]):(h,w = pWH[id])
    for posY in 0..(4-h)
      for posX in 0..(4-w)
        arr = Array.new(4).map{Array.new(4,0)}
        for i in 0..(h-1)
          for j in 0..(w-1)
            arr[posY+i][posX+j] = 1 if pTurns[id][type][i][j] == "#"
          end
        end
        pVal[id].push(arr)
      end
