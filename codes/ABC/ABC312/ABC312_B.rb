...
for pivY in 0..(n-9)
  for pivX in 0..(m-9)
    ...
    for i in 0..2
      for j in 0..2
        if s[pivY+i][pivX+j] != "#" || s[endY-i][endX-j] != "#"
          # puts "#{pivY} #{pivX}"
          # puts "#{s[pivY+i][pivX+j]} #{s[endY-i][endX-j]}"
          ifB = F
          break
        end
        break if !ifB
    ...
    ifW = T
    for i in 0..2
      if s[pivY+3][pivX+i] != "." || s[pivY+i][pivX+3] != "." || s[endY-3][endX-i] != "." || s[endY-i][endX-3] != "."
        ifW = F
        break
      end
      break if !ifW
    end
    ifW = F if s[pivY+3][pivX+3] != "." || s[endY-3][endX-3] != "."

    next if !ifW
    
    ansExist = T
    ans.push([pivY+1,pivX+1])
  end
end

ans.each{|e| puts e.join(" ")} if ansExist