...
pri = []
as.each{|e| pri.push(e[0])}
bs.each{|e| pri.push(e[0])}
pri.sort!
pri.uniq!
pri.unshift(0)
pri.push(10**12)
...
ps = Array.new(priLen).map{Array.new(2,0)}
ai = 0
for i in 1..(priLen-1)
  if pri[i] == as[ai][0]
    ps[i][0] = ps[i-1][0] + as[ai][1]
    ai += 1 if ai < as.length-1
  else
    ps[i][0] = ps[i-1][0]
  end
end
...
for jj in 1..(priLen-1)
  j = priLen-1-jj
  if pri[j] == bs[bi][0]
    ps[j][1] = ps[j+1][1] + bs[bi][1]
    bi += 1 if bi < bs.length-1
  else
    ps[j][1] = ps[j+1][1]
  end
end
...
for i in 1..(priLen-2)
  if ps[i][0] >= ps[i][1]
    if ps[i-1][0] >= ps[i][1] && ps[i][1] < ps[i-1][1]
      ans = pri[i-1]+1
    else
      ans = pri[i]
    end
