...
def bins(arr, tar, len, st, en)
  ...
  if en-st == 1
    if tar < arr[mid]
      return mid-1
    elsif arr[mid] <= tar && tar < arr[mid+1]
      return mid
    else
      return mid+1
    end
    ...
    if tar < arr[mid]
      return bins(arr,tar,len,st,mid)
    elsif tar == arr[mid]
      return mid
    else
      return bins(arr,tar,len,mid,en)
    end
  end
end
...
for qr in 1..q
  l,r = gInts
  lid = bins(a,l,n,0,n-1)
  rid = bins(a,r,n,0,n-1)
  (a[lid] == l) ? (lFull = lid):(lFull = lid+1)
  rFull = rid-1
  # puts "#{lid},#{rid} : #{lFull}:#{rFull}"
  (lid <= rid) ? (ans = pfs[rFull+1]-pfs[lFull]):(ans = 0)
  if lid % 2 == 1 && a[lid] != l && lid < n-1
    ans += a[lid+1]-l
  end
  if rid % 2 == 1 && a[rid] != r
    ans += r-a[rid]
