  ...
  for y2 in 0..upper
    for z2 in 0..upper
      for x3 in -7..upper
        for y3 in -7..upper
          for z3 in -7..upper
            c3 = calcOverlap([0,x2,x3],[0,y2,y3],[0,z2,z3])
            c2_12 = calcOverlap([0,x2],[0,y2],[0,z2])
            c2_23 = calcOverlap([x2,x3],[y2,y3],[z2,z3])
            c2_31 = calcOverlap([x3,0],[y3,0],[z3,0])
            c2 = c2_12 + c2_23 + c2_31 - c3*3
            c1 = 7**3*3 - c2*2 - c3*3
            if c1 == v1 && c2 == v2 && c3 == v3
              puts "Yes"
              puts "0 0 0 #{x2} #{y2} #{z2} #{x3} #{y3} #{z3}"
              flag = T
              break
            end
          end
