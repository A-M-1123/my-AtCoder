...
def ifAddable(froms,tos,u,v)
  return (u != v && !tos[u].has_key?(v))
end
...
froms = Array.new(n+1).map{Hash.new(0)}
tos = Array.new(n+1).map{Hash.new(0)}
...
newAddL = 0
for i in 1..n
  froms[i].each do |fk,fv|
    tos[i].each do |tk,tv|
      if ifAddable(froms,tos,fk,tk)
        newAdd.push([fk,tk])
        newAddL += 1
      end
    end
...
while curAddL > 0
  curAdd.each do |nad|
    u,v = nad
    froms[u].each do |fk,fv|
      if ifAddable(froms,tos,fk,v)
        newAdd.push([fk,v])
        newAddL += 1
      end
    end
    tos[v].each do |tk,tv|
      if ifAddable(froms,tos,v,tk)
        newAdd.push([v,tk])
        newAddL += 1
      end
    end
