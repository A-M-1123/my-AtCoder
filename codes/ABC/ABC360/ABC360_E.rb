...
dp = Array.new(k).map{Array.new(2,0)}
keep1to1 = fracMod(1, n, MOD)
moveNto1 = fracMod(n-1, n, MOD)
keepNtoN = fracMod(n-1, n, MOD)
move1toN = fracMod(1, 2, MOD)

dp[0] = [fracMod(1,n,MOD), fracMod(1,n,MOD)]
i = 0
while (i += 1) < k
  pre1 = dp[i-1][0]
  preN = dp[i-1][1]
  dp[i][0] = ((pre1 * keep1to1) % MOD + (preN * moveNto1) % MOD) % MOD
  dp[i][1] = ((preN * keepNtoN) % MOD + (pre1 * move1toN) % MOD) % MOD
end
p dp
ans = (dp[k-1][0] + (dp[k-1][1] * (sumStep(n)-1)) % MOD) % MOD
puts ans