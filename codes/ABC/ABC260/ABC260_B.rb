...
mem = Array.new(n).map{Array.new(4,0)}
for i in 0..(n-1)
  mem[i] = [a[i],b[i],a[i]+b[i],i+1]
end

pass = Array.new(n+1,F)

mem.sort!{|p,q| (q[0] <=> p[0]).nonzero? || p[3] <=> q[3]}
for i in 0..(x-1)
  pass[mem[i][3]] = T
end

mem.sort!{|p,q| (q[1] <=> p[1]).nonzero? || p[3] <=> q[3]}
cnt = 0
for i in 0..(n-1)
  break if cnt == y
  if !pass[mem[i][3]]
...
mem.sort!{|p,q| (q[2] <=> p[2]).nonzero? || p[3] <=> q[3]}
cnt = 0
for i in 0..(n-1)
  break if cnt == z
  if !pass[mem[i][3]]
