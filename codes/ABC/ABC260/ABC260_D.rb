...
def bins(a,t,st,en)
  ...
  return -1 if t < a[st] || a[en] < t
  (t <= a[m]) ? (s,e = st,m-1):(s,e = m+1,en)
  return bins(a,t,s,e)
end
  ...
  if len == 0
    ...
    if field[-1] < x
      ...
      if x < field[0]
        ...
        tarId = bins(field,x,0,len-1)
      end
      bundIdX = bundId[field[tarId]]
      bund[bundIdX].push(x)
      bundId[x] = bundIdX
      field[-1],field[tarId] = field[tarId],field[-1]
      field.pop
      field.push(x)
    end
  end
  lastBundId = bundId[field[-1]]
  # puts "try#{i+1}: <#{field.join(" ")}> #{lastBund.join(" ")}"
  if bund[lastBundId].length >= k
    bund[lastBundId].each do |e|
      ans[e] = i+1
    end
    field.pop
    len -= 1
