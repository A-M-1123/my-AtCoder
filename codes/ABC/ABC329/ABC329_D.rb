...
# makeNds(ndN,arrL,array,1way?,wgt?,lbl?,end-nil?) ex.makeNds(n,m,a,F,F,F,T)
...
finalA = Array.new(n,0)
a.each{|ae| finalA[ae-1] += 1}
maxA = finalA.max

cntA = Array.new(n,0)
topIDs = Array.new(maxA+1,10**24)
topCnt = 0
for i in 0..(m-1)
  id = a[i]-1
  cntA[id] += 1
  cnt = cntA[id]
  topIDs[cnt] = [topIDs[cnt], id].min
  topCnt = [topCnt, cntA[id]].max
  
  puts topIDs[topCnt]+1
end